                ...
                <div className="dashboard-content">
                    {showConfig ? (
                        <div className="card glass-card animate-slide-up" style={{ maxWidth: '600px', margin: '0 auto' }}>
                            ...
                            {/* Mode Toggle */}
                            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', background: '#f1f5f9', borderRadius: '10px', padding: '4px' }}>
                                <button
                                    onClick={() => setInterviewMode('chat')}
                                    style={{ flex: 1, padding: '0.6rem', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem', background: interviewMode === 'chat' ? 'white' : 'transparent', color: interviewMode === 'chat' ? '#3b82f6' : '#64748b', boxShadow: interviewMode === 'chat' ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', transition: 'all 0.2s' }}
                                ><FiMessageSquare size={16} /> Chat Mode</button>
                                <button
                                    onClick={() => setInterviewMode('voice')}
                                    style={{ flex: 1, padding: '0.6rem', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem', background: interviewMode === 'voice' ? 'linear-gradient(135deg,#3b82f6,#6366f1)' : 'transparent', color: interviewMode === 'voice' ? 'white' : '#64748b', boxShadow: interviewMode === 'voice' ? '0 2px 8px rgba(99,102,241,0.4)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', transition: 'all 0.2s' }}
                                ><FiMic size={16} /> 🎤 Voice Mode</button>
                            </div>

                            {interviewMode === 'voice' && (
                                <div style={{ background: 'linear-gradient(135deg, #eff6ff, #f5f3ff)', border: '1px solid #c7d2fe', borderRadius: '10px', padding: '1rem', marginBottom: '1rem', fontSize: '0.9rem', color: '#4338ca' }}>
                                    🎙️ <strong>Voice + Avatar Mode</strong> — An AI interviewer will ask questions aloud. Respond by speaking. Fill in company/role below then start.
                                </div>
                            )}
                                ...
                                <button
                                    onClick={() => {
                                        if (interviewMode === 'voice') {
                                            ...
                                            if (config.topic) params.set('role', config.topic);
                                    ...
                                    style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: interviewMode === 'voice' ? 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)' : 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)' }}
                                >
                                    {interviewMode === 'voice' ? <FiMic /> : <FiPlay />}
                                    {loading ? 'Starting...' : interviewMode === 'voice' ? 'Start Voice Interview 🎤' : 'Start Interview'}
                                </button>
                            </div>
                        </div>
                    ) : (
