...
import { FiMapPin, FiTrash2, FiUsers, FiLogOut, FiPlus, FiMessageSquare, FiCalendar, FiTrello, FiShield, FiTrendingUp } from 'react-icons/fi';
      ...
      <div className="dashboard-main">
        <div className="dashboard-header">
          <h1 className="dashboard-title">
            {location.pathname.includes('/post-job') ? 'Post a New Job' :
              location.pathname.includes('/jobs') ? 'My Jobs' :
                location.pathname.includes('/search') ? 'Find Candidates' :
                  location.pathname.includes('/matches') ? 'Smart Matches' :
                    'Recruiter Workspace'}
          </h1>
        </div>
        <div className="dashboard-content">
          ...
          {(location.pathname === '/dashboard/recruiter' || location.pathname.includes('/post-job')) && (
          ...
          {(location.pathname === '/dashboard/recruiter' || location.pathname.includes('/jobs')) && (
                  ...
                  <div key={job.id} className="job-item">
                    ...
                    <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                      ...
                      <button
                        className="form-button"
                        type="button"
                        onClick={() => navigate(`/recruiter/pipeline?jobId=${job.id}`)}
                        style={{ width: 'fit-content' }}
                      >
          ...
          {location.pathname.includes('/search') && (
            <div className="empty-state" style={{ marginTop: '2rem', padding: '4rem', textAlign: 'center', background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
              <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🔍</div>
              <h2>Candidate Search</h2>
              <p style={{ color: '#7f8c8d', maxWidth: '400px', margin: '0 auto' }}>Advanced candidate search based on skills, experience, and education is coming soon.</p>
            </div>
          )}
          ...
          {location.pathname.includes('/matches') && (
            <div className="empty-state" style={{ marginTop: '2rem', padding: '4rem', textAlign: 'center', background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
              ...
              <p style={{ color: '#7f8c8d', maxWidth: '400px', margin: '0 auto' }}>Proactive AI recommendations for candidates who are a perfect fit for your open roles are coming soon.</p>
            </div>
          )}
        </div>
      </div>
