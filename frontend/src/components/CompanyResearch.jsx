...
import { FiSearch, FiBriefcase, FiUsers, FiCode, FiTrendingUp, FiExternalLink, FiBookOpen, FiArrowLeft } from 'react-icons/fi';
import '../App.css';

const CompanyResearch = () => {
    const navigate = useNavigate();
    const { user } = useAuth();
    const [company, setCompany] = usePersistedState('research_company', '');
    const [loading, setLoading] = useState(false);
    const [research, setResearch] = usePersistedState('research_result', null, { maxAgeMs: 24 * 60 * 60 * 1000 });
    const [activeTab, setActiveTab] = usePersistedState('research_activeTab', 'overview');
    const [recentSearches, setRecentSearches] = usePersistedState('research_recentSearches', []);
                        ...
                        <div className="card">
                            ...
                            <div style={{
                                ...
                                <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.75rem' }}>
                                    <button
                                        onClick={() => navigate('/opportunities?tab=jobs', { state: { filterCompany: research.company_name || company } })}
